...
${r.text}`,a=(await e.chatCompletions({messages:[{role:"system",content:vr},{role:"user",content:n}],stream:!1})).choices[0]?.message?.content||"";return _r(a,r.title)}catch(t){return console.error("[CalmWeb] API analysis failed:",t),Ve(r,t.message||"Analysis failed")}}function _r(r,e){const t=r.trim();let n=t;const s=t.match(/```(?:json)?\s*\n?([\s\S]*?)\n?```/);s&&(n=s[1]);const a=n.match(/\{[\s\S]*\}/);if(a)try{const i=JSON.parse(a[0]);return{success:!0,title:i.title||e,summary:i.summary||"",filteredContent:i.filteredContent||"",confidence:i.confidence??.5,decisions:Array.isArray(i.decisions)?i.decisions:[],error:void 0}}catch{}return{success:!1,title:e,summary:"",filteredContent:t.slice(0,1e4),confidence:0,decisions:[],error:"Failed to parse AI response as JSON"}}function Ve(r,e){return{success:!1,title:r.title,summary:"",filteredContent:r.text.slice(0,1e4),confidence:0,decisions:[],error:e}}async function ht(){const r=ft();if(!r)return{total:0,used:0,remaining:0,isPaidUser:!1};try{const[e,t]=await Promise.all([r.getUser(),r.getQuota()]),n=e.subscription?.active||!1,s=n?t.remaining:0;return{total:t.total,used:t.used,remaining:s,isPaidUser:n}}catch(e){return console.error("[CalmWeb] Failed to get quota:",e),{total:0,used:0,remaining:0,isPaidUser:!1}}}const De="calmweb-reader-host";async function Ee(r={}){document.body.style.setProperty("overflow","hidden","important"),me();const e=r.mode||"full";let t=null,n=null;const s=await ht();if(console.log("[CalmWeb] Quota check:",s.isPaidUser?"Paid user":"Free user","- Remaining:",s.remaining),s.isPaidUser&&s.remaining>0){console.log("[CalmWeb] Using AI-powered analysis...");try{n=await wr({title:document.title,url:window.location.href,html:document.body?.innerHTML?.slice(0,15e3)||"",text:document.body?.textContent?.slice(0,8e3)||""}),n.success?console.log("[CalmWeb] API analysis succeeded"):console.log("[CalmWeb] API analysis failed:",n.error)}catch(k){console.error("[CalmWeb] API analysis error:",k)}}else console.log("[CalmWeb] No AI - using CSS extraction (free tier)");if(!n?.success)try{t=qt(document,window.location.href,e)}catch(k){console.error("[CalmWeb] Extraction failed:",k)}const a=n?.title||t?.title||document.title||"Current Page",i=r.layoutId?tr(r.layoutId):it,o=document.createElement("div");o.id=De,o.style.cssText=`
  ...
  `,b.appendChild(h);const A=document.createElement("div");A.className="cw-overlay";const j=document.createElement("div");j.className="cw-bg-pattern";const D=document.createElement("div");D.className="cw-toolbar",D.innerHTML=`
...
`;function yn(){const r=window.location.hostname.toLowerCase(),e=window.location.pathname.toLowerCase();return!!(["mail.google","calendar.google","drive.google","docs.google","sheets.google","slides.google","meet.google","github.com","gitlab.com","slack.com","discord.com","notion.so","figma.com","linear.app","trello.com","netflix.com","spotify.com","youtube.com"].some(a=>r.includes(a))||["/login","/signin","/signup","/register","/auth","/account","/settings","/admin","/dashboard","/checkout","/cart"].some(a=>e.startsWith(a))||document.querySelectorAll('input:not([type="hidden"]):not([type="search"]), textarea, select').length>4)}function Ot(){try{if(document.getElementById(Ze))return;document.body.style.setProperty("overflow","hidden","important");const r=document.createElement("div");r.id=Ze,r.innerHTML=`<style>${pn}</style>
      <div class="calmweb-loading-spinner"></div>
      <div class="calmweb-loading-text">Filtering...</div>`,document.body.appendChild(r)}catch(r){console.error("[CalmWeb] Loading screen failed:",r)}}function nt(){document.getElementById(Ze)?.remove()}function ve(){if(document.getElementById(rt))return;const r=document.createElement("div");r.id=rt,r.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',r.title="Filtered View (Ctrl+Shift+R)",Object.assign(r.style,{position:"fixed",bottom:"20px",right:"20px",width:"40px",height:"40px",borderRadius:"50%",background:"#8b5cf6",color:"white",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",zIndex:"2147483646",boxShadow:"0 4px 24px rgba(139, 92, 246, 0.3)",transition:"transform 0.15s ease",border:"none"}),r.addEventListener("mouseenter",()=>{r.style.transform="scale(1.1)"}),r.addEventListener("mouseleave",()=>{r.style.transform="scale(1)"}),r.addEventListener("click",()=>{r.remove(),Ot(),setTimeout(()=>{nt();try{Ee()}catch(e){console.error(e)}},300)}),document.body.appendChild(r)}function Mt(){document.getElementById(rt)?.remove()}function st(){try{Fe()?(me(),ve()):(Mt(),Ee())}catch(r){console.error("[CalmWeb] Reader toggle failed:",r)}}const bn={matches:["<all_urls>"],runAt:"document_idle",async main(){if(console.log("[CalmWeb] Reader loaded on",window.location.hostname),hn()){console.log("[CalmWeb] Skipping reader on Google search page");return}document.addEventListener("keydown",t=>{t.ctrlKey&&t.shiftKey&&!t.altKey&&!t.metaKey&&t.key.toLowerCase()==="r"&&(t.preventDefault(),t.stopPropagation(),st()),t.altKey&&!t.ctrlKey&&!t.metaKey&&!t.shiftKey&&t.key.toLowerCase()==="r"&&(t.preventDefault(),t.stopPropagation(),st())},!0),mn.runtime.onMessage.addListener(t=>{if(t.type===H.TOGGLE_READER&&st(),t.type===H.OPEN_READER){Mt();try{Fe()||Ee()}catch(n){console.error(n)}}if(t.type===H.CLOSE_READER)try{Fe()&&(me(),ve())}catch(n){console.error(n)}});let r=!0,e={};try{const t=cn({type:H.GET_SETTINGS}),n=new Promise((a,i)=>setTimeout(()=>i(new Error("Settings timeout")),2e3)),s=await Promise.race([t,n]);(s?.reader?.autoOpen===!1||s?.enabled===!1)&&(r=!1),e=s?.reader||{},console.log("[CalmWeb] Settings loaded, filtering:",r)}catch(t){console.warn("[CalmWeb] Settings load failed, defaulting to filter:",t)}if(!r){ve();return}if(yn()){ve();return}console.log("[CalmWeb] Opening reader..."),Ot(),Ee({mode:e.mode||"full",layoutId:e.defaultLayout,font:e.font,fontSize:e.fontSize}).then(()=>{nt(),console.log("[CalmWeb] Reader opened OK")}).catch(t=>{console.error("[CalmWeb] Reader failed:",t),nt(),ve()})}};function xn(){}function ze(r,...e){}const at={debug:(...r)=>ze(console.debug,...r),log:(...r)=>ze(console.log,...r),warn:(...r)=>ze(console.warn,...r),error:(...r)=>ze(console.error,...r)};return(()=>{try{}catch(e){throw at.error('Failed to initialize plugins for "reader"',e),e}let r;try{r=bn.main(),r instanceof Promise&&(r=r.catch(e=>{throw at.error('The unlisted script "reader" crashed on startup!',e),e}))}catch(e){throw at.error('The unlisted script "reader" crashed on startup!',e),e}return r})()})();
reader;