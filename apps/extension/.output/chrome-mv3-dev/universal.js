...
`);
  ...
  `;
	...
	//#endregion
	//#region src/adapters/universal.ts
	var IGNORE_PARENTS = [
		"nav",
		"header",
		"footer",
	...
	*/
	function discoverUnitsUniversal(root) {
		const candidates = [];
		const seen = /* @__PURE__ */ new Set();
		const addCandidate = (el, text, forceAdd = false) => {
			if (!el) return;
		...
		for (const link of Array.from(links)) {
			...
			if (container) addCandidate(container, text);
			else addCandidate(link, text);
		}
		...
		for (const img of Array.from(images)) {
			...
			addCandidate(img, alt || `Image: ${src.substring(0, 20)}`, true);
		}
		const videos = root.querySelectorAll("video, iframe[src*=\"youtube\"], iframe[src*=\"vimeo\"], iframe[src*=\"video\"]");
		for (const video of Array.from(videos)) addCandidate(video, getText(video) || video.getAttribute("title") || video.getAttribute("src") || "", true);
		const listItems = root.querySelectorAll("li");
		for (const li of Array.from(listItems)) {
			const text = getText(li);
			if (text.length < 15) continue;
			addCandidate(li, text);
		}
		...
		for (const div of Array.from(divs)) {
			...
			addCandidate(div, text.substring(0, 100));
		}
	...
	*/
	function extractTitle(element) {
		...
		if (element.tagName === "VIDEO" || element.tagName === "IFRAME") return element.getAttribute("title") || element.getAttribute("src") || "Video";
		for (const sel of [
			"h1",
			"h2",
			"h3",
		...
		if (trimmed.length > 0) return trimmed.substring(0, 100);
		return "Untitled";
	}
	/**
	...
	var universalAdapter = {
		...
		applyDecision(element, result) {
			...
			if (result.decision === "hide") {
				...
				if (titleEl instanceof HTMLElement) titleEl.innerText = result.neutralizedTitle;
				element.setAttribute("data-calmweb-processed", "neutralized");
				window.dispatchEvent(new CustomEvent("calmweb:neutralized", { detail: {
					before: originalTitle,
					after: result.neutralizedTitle
				} }));
			} else element.setAttribute("data-calmweb-processed", "show");
		}
	};
	//#endregion
	...
	*/
	function hideAds(root = document) {
		...
		for (const selector of AD_SELECTORS) try {
			...
			for (const el of Array.from(elements)) if (el instanceof HTMLElement) {
	...
	*/
	function observeAndHideAds(root = document) {
		...
		const observer = new MutationObserver((mutations) => {
			...
			if (hasNewElements) hideAds(root);
		});
...
`;
  ...
  `;
	...
	//#endregion
	//#region entrypoints/universal.ts
	/**
	...
	*/
	var universal_default = defineContentScript({
		matches: ["<all_urls>"],
		runAt: "document_idle",
		main() {
    ...
    `;
			...
			new MutationObserver((mutations) => {
				for (const mutation of mutations) if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
					for (const node of Array.from(mutation.addedNodes)) if (node instanceof HTMLElement) {
						const unprocessed = universalAdapter.discoverUnits(node).filter((el) => !el.hasAttribute("data-calmweb-processed"));
						if (unprocessed.length > 0) processUnits(unprocessed);
					}
				}
			...
			setInterval(() => {
				const unprocessed = universalAdapter.discoverUnits(document).filter((el) => !el.hasAttribute("data-calmweb-processed"));
				if (unprocessed.length > 0) processUnits(unprocessed);
			}, 5e3);
	...
	//#endregion
	//#region \0virtual:wxt-unlisted-script-entrypoint?/home/dracon/Dev/extensions/calmweb/apps/extension/entrypoints/universal.ts
